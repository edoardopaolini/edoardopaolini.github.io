...
- The cortical surface is the real ICBM152 template (BrainMesh_ICBM152 of BrainNet Viewer, surface by Prof. Alan C. Evans, MNI), prepared offline into `assets/js/cortex-data.js`: 9531 points, one per 4.4 mm cell of the 81924-vertex mesh, each carrying the mean curvature of the full-resolution folded surface (positive on gyral crowns, negative in sulci). One big-endian uint32 per point, 9 bits per axis plus 5 bits of curvature, base64; `centre` and `scale` convert MNI millimetres to model units. Attribution lives in the file header and in the README.
- Rendering: the surface normal is the radial direction (chosen over the true mesh normals, which read as noise at this size); tone `v = (0.13 + 0.87*lambert) * sulcal * depth`, with the curvature carrying the folds. The point cloud is drawn into an offscreen canvas and reused, re-rendered only past a 1 degree yaw change, a pitch change, a resize, a theme change or a change of the highlighted zone. Per frame: back edges, the cached cortex, front edges, sources, one label.
- Candidate zone (the epileptogenic area, drawn on the cortex): seed at MNI (-52, -48, 26), radius 32 mm, left hemisphere; 556 points, weighted centroid at MNI (-52, -46, 26), left temporo-parietal. Points inside are tinted toward the accent in proportion to their weight, with a soft accent glow behind them that fades as the patch turns away.
- Network: 24 cortical sources, no EEG electrodes. Six inside the zone (farthest-point sampled from the seed) and eighteen over the cortex, at least 0.22 model units apart. Edges `exp(-(d/0.55)^2) * (0.55 + 0.45*rand)` with `Lab.rng(20260906)`, symmetric, multiplied by 2.5 when both ends are in the zone, the 52 strongest kept. Arcs lift off the surface and are drawn behind or in front of the cortex according to their depth, so occlusion reads.
