...
- The cortical surface is the real ICBM152 template (BrainMesh_ICBM152 of BrainNet Viewer, surface by Prof. Alan C. Evans, MNI), prepared offline into `assets/js/cortex-data.js` as a low-poly mesh: 1284 vertices and 2560 faces. The template is two hemispheres, each an icosphere subdivided six times with the subdivision nested in the vertex order, so inverting it three times gives an exact, hole-free coarse mesh with no decimation artefacts. Vertex positions come from the Taubin-smoothed surface; the anatomy is put back by two per-vertex fields, the mean curvature of the full-resolution folded surface averaged over each coarse cell, and normals sampled from the full-resolution surface. The file also carries the per-vertex zone weight and the 24 source indices. Attribution lives in the file header and in the README.
- Rendering: filled triangles, flat-shaded from the mean of their vertex normals, tone `v = (0.16 + 0.84*lambert) * sulcal * depth` with the curvature darkening the sulci. Faces are culled by the sign of their projected area (the sampled normals differ too much from the plane of their coarse triangle to cull with), sorted back to front and filled in quantised tone buckets, one path per bucket, so a few dozen fills cover the whole surface; each triangle is grown 0.4 px from its centroid, which closes the seams far more cheaply than stroking. The surface is drawn into an offscreen canvas and reused, re-rendered only past a 0.5 degree yaw change, a pitch change, a resize or a theme change. Per frame: back edges, the cached cortex, front edges, sources, one label.
