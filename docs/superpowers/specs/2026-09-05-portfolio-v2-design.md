...
- The cortical surface is the real ICBM152 template (BrainMesh_ICBM152_smoothed of BrainNet Viewer, surface by Prof. Alan C. Evans, MNI), prepared offline into `assets/js/cortex-data.js` as a low-poly mesh: 1284 vertices and 2560 faces. The template is two hemispheres, each an icosphere subdivided six times with the subdivision nested in the vertex order, so inverting it three times gives an exact, hole-free coarse mesh with no decimation artefacts. Eight Taubin passes on the coarse mesh remove the spikes a 10 mm sampling leaves on a folded surface, and each hemisphere is wound outward so the stored normals face out. The anatomy comes back through two per-vertex fields: the mean curvature of the full-resolution folded surface averaged over each coarse cell, which darkens the sulci without the geometry carrying them, and the normals of the smoothed coarse mesh, which keep the shading consistent with the shape. The file also carries the per-vertex zone weight, computed from the anatomical positions of the unsmoothed template, and the 24 source indices. Attribution lives in the file header and in the README.
