...
9. **Outside the lab** (`#beyond`, collage: four columns, the first and fourth photos span two columns, the piano spans two rows, the last text tile spans two columns so the grid has no hole; two columns under 900 px, one under 520 px). H2 `t.beyond.title`. Items from `beyond.yml`: eight photo tiles (skiing at Obereggen, piano, football, running, volleyball, reading, music, good company); an item without a photo falls back to a text tile. Notes exist only where a source supports them. A `<details>` "Photo credits" lists the Wikimedia Commons authors and licences, each linked to the file page (CC BY-SA 4.0 requires attribution and a licence link).
...
### brain.js (home) and cortex-data.js

- The cortical surface is the real ICBM152 template (BrainMesh_ICBM152_smoothed of BrainNet Viewer, surface by Prof. Alan C. Evans, MNI), prepared offline into `assets/js/cortex-data.js` as a low-poly mesh: 1284 vertices and 2560 faces. The template is two hemispheres, each an icosphere subdivided six times with the subdivision nested in the vertex order, so inverting it three times gives an exact, hole-free coarse mesh with no decimation artefacts. Eight Taubin passes on the coarse mesh remove the spikes a 10 mm sampling leaves on a folded surface, and each hemisphere is wound outward so the stored normals face out. The anatomy comes back through two per-vertex fields: the mean curvature of the full-resolution folded surface averaged over each coarse cell, then smoothed over the coarse mesh and renormalised, which darkens whole sulcal bands without the geometry carrying them (the raw field alternates from vertex to vertex at this spacing, and a speckled surface reads as a transparent one), and the normals of the smoothed coarse mesh, which keep the shading consistent with the shape. The file also carries the per-vertex zone weight, computed from the anatomical positions of the unsmoothed template, and the 24 source indices. Attribution lives in the file header and in the README.
- Rendering: filled triangles, flat-shaded from the mean of their vertex normals, tone `v = (0.16 + 0.84*lambert) * sulcal * depth` with the curvature darkening the sulci. The tone becomes a colour on a ramp between the two theme tokens, and the ramp runs the other way in the light theme, so the lit side is the palest in both; without that flip the light theme reads as a negative. Faces are culled by the sign of their projected area (the sampled normals differ too much from the plane of their coarse triangle to cull with), sorted back to front and filled in quantised tone buckets, one path per bucket, so a few dozen fills cover the whole surface; each triangle is grown 0.4 px from its centroid, which closes the seams far more cheaply than stroking. The surface is drawn into an offscreen canvas and reused, re-rendered only past a 0.5 degree yaw change, a pitch change, a resize or a theme change. Per frame: back edges, the cached cortex, front edges, sources, one label.
- Candidate zone (the epileptogenic area, drawn on the cortex): seed at MNI (-52, -48, 26), radius 32 mm, left hemisphere, computed offline from the anatomical vertex positions; 77 vertices carry it, centroid MNI (-51, -47, 24). Faces inside are tinted toward the accent in proportion to their weight, so the patch reads as painted cortex, with a soft accent glow behind them that fades as it turns away.
- Network: 24 cortical sources, no EEG electrodes, chosen offline as mesh vertices: six inside the zone (farthest-point sampled from the seed) and eighteen over the cortex, at least 0.27 model units apart. Edges `exp(-(d/0.55)^2) * (0.55 + 0.45*rand)` with `Lab.rng(20260906)`, symmetric, multiplied by 2.5 when both ends are in the zone, the 52 strongest kept. Arcs lift off the surface and are drawn behind or in front of the cortex according to their depth, so occlusion reads.
- Interaction unchanged from the earlier figure: one real button per source, hidden and disabled when it faces away, hover, focus and pin, drag and arrow keys to rotate, idle rotation held by hover, drag or keyboard focus, `?highlight=` and `?yaw=`, reduced motion draws one frame per interaction.
